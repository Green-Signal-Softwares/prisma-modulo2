...
<style>
    ...
    @keyframes vcPulse {
        0%, 100% { box-shadow: 0 4px 16px rgba(218,41,28,0.4); transform: scale(1); }
        50% { box-shadow: 0 6px 24px rgba(218,41,28,0.7); transform: scale(1.05); }
    }
...
<div class="w-full flex-1 bg-[#F5F6F8] flex items-stretch select-none overflow-hidden min-h-0" style="font-family: 'AMX', sans-serif;">
    
    <!-- COLUNA ESQUERDA: LISTA DE CONVERSAS -->
    <div class="w-[320px] lg:w-[350px] bg-white flex flex-col flex-shrink-0 relative border-r border-gray-200 select-none overflow-hidden">
        ...
        <div class="px-6 mb-4">
            <div class="bg-gray-200/70 p-1 rounded-xl flex items-center justify-between gap-1">
                <button id="filter-all-btn" onclick="setChatFilter('all')" class="flex-1 py-1.5 rounded-lg text-xs font-bold bg-[#DA291C] text-white transition-all focus:outline-none cursor-pointer text-center shadow-sm">
                    Todas mensagens
                </button>
                <button id="filter-unread-btn" onclick="setChatFilter('unread')" class="flex-1 py-1.5 rounded-lg text-xs font-bold bg-transparent text-gray-600 hover:text-gray-900 transition-all focus:outline-none cursor-pointer text-center">
                    Mensagens não lidas
                </button>
            </div>
        </div>
            ...
            <div class="flex-1 overflow-y-auto pb-6 chat-thread-list border-t border-gray-200">
                @forelse($solicitations as $sol)
                    ...
                    @endphp
                    <a 
                        href="{{ auth()->user()->role === 'atendente' ? route('atendente.chat.index', $sol->id) : route('chat.index', $sol->id) }}"
                        ...
                        data-title="{{ strtolower($sol->title) }}"
                    ...
                    >
                        ...
                        @if(!$targetUser && auth()->user()->role === 'user')
                            ...
                            <div class="w-12 h-12 rounded-full bg-white flex items-center justify-center flex-shrink-0 border p-2.5 {{ $isActive ? 'border-white shadow-sm' : 'border-gray-300 shadow-sm' }}">
                        ...
                        @else
                            ...
                            <div class="relative flex-shrink-0 w-12 h-12">
                                <img src="https://ui-avatars.com/api/?name={{ urlencode($avatarName) }}&background={{ $avatarBg }}&color={{ $avatarColor }}&bold=true&rounded=true" alt="Avatar" class="w-12 h-12 rounded-full object-cover border {{ $isActive ? 'border-white' : 'border-gray-400' }}">
                        ...
                        <div class="flex-1 min-w-0">
                            <h4 class="text-[13px] font-extrabold truncate pr-1 {{ $isActive ? 'text-white' : 'text-gray-900' }}">
                                {{ $sol->title }}
                            </h4>
                        ...
                        <div class="flex flex-col items-end gap-1.5 flex-shrink-0 font-bold">
                            ...
                            @if($hasUnread)
                                <div class="w-4 h-4 bg-[#DA291C] rounded-full flex items-center justify-center text-[9px] font-bold text-white shadow-sm">
    ...
    <div class="flex-1 p-6 flex justify-between items-stretch gap-6 overflow-hidden min-h-0">
            ...
            <div class="flex-1 min-w-[500px] bg-white rounded-[24px] shadow-md flex flex-col justify-between border border-gray-200/80 overflow-hidden pb-4 min-h-0">
        @if($activeSolicitation)
            ...
            <div class="h-16 px-6 bg-white border-b border-gray-100 flex items-center justify-between select-none">
                <div class="flex items-center gap-3">
                    ...
                    @if(!$activeAvatarUser && auth()->user()->role === 'user')
                        ...
                        <div class="w-10 h-10 rounded-full bg-white flex items-center justify-center flex-shrink-0 border p-2 shadow-sm">
                            <img src="/icones/Icone Ticket.png" alt="Ticket" class="w-full h-full object-contain">
                        </div>
                    @else
                        <div class="relative w-10 h-10 flex-shrink-0">
                            <img src="https://ui-avatars.com/api/?name={{ urlencode($activeAvatarName) }}&background={{ $activeAvatarBg }}&color={{ $activeAvatarColor }}&bold=true&rounded=true" alt="Avatar" class="w-10 h-10 rounded-full object-cover border border-gray-200">
                    ...
                    <div class="flex items-center gap-1.5">
                        ...
                        <span class="w-4 h-4 bg-[#DA291C] rounded-full flex items-center justify-center text-[9px] font-bold text-white">3</span>
                        <span class="text-xs font-bold text-gray-550">@</span>
                        <span id="current-ticket-tag" class="hidden items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-bold text-white uppercase"></span>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-4 text-gray-700">
                    @if(auth()->user()->role === 'atendente')
                        <!-- 1. Swap/Transfer -->
                        <div class="relative">
                            <button id="btn-transfer-menu" onclick="toggleTransferMenu(event)" class="hover:opacity-85 transition-opacity focus:outline-none cursor-pointer flex items-center justify-center" title="Transferir chamado">
                                <img src="/icones/Icone Transferir.png" alt="Transferir" class="w-5.5 h-5.5 object-contain">
                            </button>
                            
                            <!-- Dropdown de Transferência -->
                            <div id="transfer-dropdown" class="hidden absolute top-full right-0 mt-3 z-50 w-72 bg-[#2D2D30] text-white rounded-2xl shadow-2xl border border-white/10 flex flex-col max-h-[480px] overflow-hidden transition-all duration-200">
                                ...
                                <div class="p-3 border-b border-white/5">
                                    <div class="relative">
                                        ...
                                        <div class="absolute right-3.5 top-2 w-4 h-4 opacity-40">
                                            <img src="/icones/Icone Buscar.png" alt="Buscar" class="w-full h-full object-contain invert">
                                        </div>
                                    </div>
                                </div>
                                
                                <!-- Lista de Itens (Scrollable) -->
                                <div class="flex-1 overflow-y-auto p-3 space-y-4 chat-scroll-clean text-left select-none max-h-[380px]">
                                    <!-- Setor -->
                                    <div>
                                        <div class="flex items-center gap-2 text-white/50 text-[10px] font-extrabold tracking-wider uppercase mb-1.5 px-2">
                                        ...
                                        <div class="space-y-0.5">
                                            <button onclick="transferTo('Setor', 'Técnico')" class="transfer-item w-full text-left px-3 py-1.5 rounded-lg text-xs font-semibold text-white/90 hover:bg-white/10 hover:text-white transition-all">
                                                Técnico
                                            </button>
                                            <button onclick="transferTo('Setor', 'Comercial')" class="transfer-item w-full text-left px-3 py-1.5 rounded-lg text-xs font-semibold text-white/90 hover:bg-white/10 hover:text-white transition-all">
                                                Comercial
                                            </button>
                                            <button onclick="transferTo('Setor', 'Suporte')" class="transfer-item w-full text-left px-3 py-1.5 rounded-lg text-xs font-semibold text-white/90 hover:bg-white/10 hover:text-white transition-all">
                                                Suporte
                                            </button>
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        <div class="flex items-center gap-2 text-white/50 text-[10px] font-extrabold tracking-wider uppercase mb-1.5 px-2">
                                            <svg class="w-3.5 h-3.5 text-white/50" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 12.75V12A2.25 2.25 0 0 1 4.5 9.75h15A2.25 2.25 0 0 1 21.75 12v.75m-8.625-5.25L12 3m0 0-1.125 1.5M12 3v13.5m0-13.5h.008v.008H12V3Zm0 13.5v.75m0-.75a2.25 2.25 0 0 1-2.25-2.25v-.75m2.25 3a2.25 2.25 0 0 0 2.25-2.25v-.75" />
                                            </svg>
                                            Filas
                                        </div>
                                        <div class="space-y-0.5">
                                            <!-- Fila 1 -->
                                            <div>
                                                <div onclick="toggleSubmenu('fila-1-sub', event)" class="transfer-item flex items-center justify-between w-full text-left px-3 py-1.5 rounded-lg text-xs font-semibold text-white/90 hover:bg-white/10 hover:text-white transition-all cursor-pointer">
                                                    <span>Fila 1</span>
                                                    <svg class="w-3 h-3 text-white/60 transition-transform duration-200" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
                                                    </svg>
                                                </div>
                                                <div id="fila-1-sub" class="hidden pl-4 pr-1 py-1 space-y-0.5 bg-black/10 rounded-lg mt-0.5">
                                                    <button onclick="transferTo('Fila 1', 'Nível 1')" class="w-full text-left px-2.5 py-1 rounded text-[11px] font-semibold text-white/80 hover:bg-white/10 hover:text-white transition-all">Nível 1</button>
                                                    <button onclick="transferTo('Fila 1', 'Nível 2')" class="w-full text-left px-2.5 py-1 rounded text-[11px] font-semibold text-white/80 hover:bg-white/10 hover:text-white transition-all">Nível 2</button>
                                                </div>
                                            </div>
                                            <!-- Fila 2 -->
                                            <div>
                                                <div onclick="toggleSubmenu('fila-2-sub', event)" class="transfer-item flex items-center justify-between w-full text-left px-3 py-1.5 rounded-lg text-xs font-semibold text-white/90 hover:bg-white/10 hover:text-white transition-all cursor-pointer">
                                                    <span>Fila 2</span>
                                                    <svg class="w-3 h-3 text-white/60 transition-transform duration-200" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
                                                    </svg>
                                                </div>
                                                <div id="fila-2-sub" class="hidden pl-4 pr-1 py-1 space-y-0.5 bg-black/10 rounded-lg mt-0.5">
                                                    <button onclick="transferTo('Fila 2', 'Nível 1')" class="w-full text-left px-2.5 py-1 rounded text-[11px] font-semibold text-white/80 hover:bg-white/10 hover:text-white transition-all">Nível 1</button>
                                                    <button onclick="transferTo('Fila 2', 'Nível 2')" class="w-full text-left px-2.5 py-1 rounded text-[11px] font-semibold text-white/80 hover:bg-white/10 hover:text-white transition-all">Nível 2</button>
                                                </div>
                                            </div>
                                            <!-- Fila 3 -->
                                            <div>
                                                <div onclick="toggleSubmenu('fila-3-sub', event)" class="transfer-item flex items-center justify-between w-full text-left px-3 py-1.5 rounded-lg text-xs font-semibold text-white/90 hover:bg-white/10 hover:text-white transition-all cursor-pointer">
                                                    <span>Fila 3</span>
                                                    <svg class="w-3 h-3 text-white/60 transition-transform duration-200" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
                                                    </svg>
                                                </div>
                                                <div id="fila-3-sub" class="hidden pl-4 pr-1 py-1 space-y-0.5 bg-black/10 rounded-lg mt-0.5">
                                                    <button onclick="transferTo('Fila 3', 'Nível 1')" class="w-full text-left px-2.5 py-1 rounded text-[11px] font-semibold text-white/80 hover:bg-white/10 hover:text-white transition-all">Nível 1</button>
                                                    <button onclick="transferTo('Fila 3', 'Nível 2')" class="w-full text-left px-2.5 py-1 rounded text-[11px] font-semibold text-white/80 hover:bg-white/10 hover:text-white transition-all">Nível 2</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div class="space-y-0.5">
                                            <!-- Ana Silva -->
                                            <button onclick="transferTo('Pessoa', 'ANA SILVA')" class="transfer-item w-full flex items-center gap-2.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-white/90 hover:bg-white/10 hover:text-white transition-all">
                                                <img src="https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=100&auto=format&fit=crop&q=80" alt="Ana Silva" class="w-6 h-6 rounded-full object-cover">
                                                <span>ANA SILVA</span>
                                            </button>
                                            <!-- Abner Junior -->
                                            <button onclick="transferTo('Pessoa', 'ABNER JÚNIOR')" class="transfer-item w-full flex items-center gap-2.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-white/90 hover:bg-white/10 hover:text-white transition-all">
                                                <img src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=100&auto=format&fit=crop&q=80" alt="Abner Junior" class="w-6 h-6 rounded-full object-cover">
                                                <span>ABNER JÚNIOR</span>
                                            </button>
                                            <!-- Bruna Ferreira -->
                                            <button onclick="transferTo('Pessoa', 'BRUNA FERREIRA')" class="transfer-item w-full flex items-center gap-2.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-white/90 hover:bg-white/10 hover:text-white transition-all">
                                                <div class="w-6 h-6 rounded-full bg-white/20 flex items-center justify-center flex-shrink-0 text-[10px] font-bold text-white">BF</div>
                        ...
                        <!-- 2. PIP/Expand -->
                        <button class="hover:text-[#DA291C] transition-colors focus:outline-none cursor-pointer" title="Minimizar/Expandir">
                            <svg class="w-5.5 h-5.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.2" stroke="currentColor">
                        ...
                        <!-- 3. Tag/Label -->
                        <button id="tag-menu-trigger" onclick="toggleTagMenu(event)" class="hover:opacity-85 transition-opacity focus:outline-none cursor-pointer flex items-center justify-center" title="Adicionar etiqueta">
                            <img src="/icones/Icone Tag.png" alt="Tag" class="w-5.5 h-5.5 object-contain">
                        </button>
                        <!-- 4. Pesquisar -->
                        <button onclick="toggleChatSearch()" class="hover:opacity-85 transition-opacity focus:outline-none cursor-pointer flex items-center justify-center" title="Pesquisar mensagens">
                            <img src="/icones/Icone Buscar.png" alt="Buscar" class="w-5 h-5 object-contain">
                        </button>
                        <!-- 5. Chamada de vídeo -->
                        <button
                            id="btn-videocall-atendente"
                            onclick="initiateVideoCall({{ $activeSolicitation->id }})"
                        ...
                        >
                            <img src="/icones/Icone Video Chamada.png" alt="Vídeo Chamada" class="w-5.5 h-5.5 object-contain">
                        </button>
                        <!-- 6. Chamada de voz -->
                        <button class="hover:opacity-85 transition-opacity focus:outline-none cursor-pointer relative flex items-center justify-center" title="Iniciar chamada de voz">
                            <img src="/icones/Icone Chamada.png" alt="Chamada de Voz" class="w-5.5 h-5.5 object-contain">
                            <span class="absolute -top-1.2 -right-1 text-[9px] font-extrabold text-gray-700 bg-white rounded-full px-0.5 border border-white">+</span>
                        </button>
                        <!-- 7. Finalizar atendimento -->
                        <button type="button" onclick="openChecklistModal()" class="hover:text-[#DA291C] transition-colors focus:outline-none cursor-pointer" title="Finalizar atendimento">
            ...
            <div id="chat-search-bar" class="hidden px-6 py-3 bg-[#EDEDED] border-b border-gray-300 flex items-center justify-between gap-4 transition-all">
                <div class="relative flex-1">
                    ...
                    <button id="chat-search-clear" onclick="document.getElementById('chat-search-input').value = ''; filterChatMessages(''); document.getElementById('chat-search-input').focus();" class="absolute right-3.5 top-2 text-gray-400 hover:text-gray-600 focus:outline-none hidden">
                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                    </button>
                </div>
                <button onclick="toggleChatSearch()" class="text-xs text-gray-500 hover:text-gray-800 font-extrabold transition-colors">
                    FECHAR
                </button>
            </div>
            ...
            <div class="flex-1 overflow-y-auto p-6 space-y-6 chat-scroll-clean" id="chat-messages-container">
                @if($activeSolicitation)
                    ...
                    <div class="group flex items-center {{ auth()->user()->role === 'atendente' ? 'justify-start mr-auto' : 'justify-end ml-auto' }} gap-3 max-w-[85%] relative chat-message" data-message-id="opening">
                        <!-- Bubble wrapper -->
                        <div class="flex flex-col {{ auth()->user()->role === 'atendente' ? 'items-start' : 'items-end' }} gap-1 max-w-[90%] relative">
                            <div class="relative p-4 {{ auth()->user()->role === 'atendente' ? 'bg-[#EDEDED] text-gray-800 rounded-2xl rounded-tl-none border border-transparent' : 'text-white bg-[#DA291C] rounded-2xl rounded-tr-none' }} shadow-md flex flex-col gap-2 pr-8 message-bubble-content transition-all duration-300">
                                ...
                                <div class="flex justify-between items-center text-[9px] font-extrabold tracking-wider {{ auth()->user()->role === 'atendente' ? 'text-gray-500 border-gray-300/30' : 'text-white/90 border-white/10' }} uppercase border-t pt-1.5 gap-4">
                                    <span class="message-sender">{{ strtoupper($activeSolicitation->user->name ?? 'Cliente') }}</span>
                                    <span class="message-time">{{ $activeSolicitation->created_at->format('d/m - H:i') }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    @if($activeSolicitation->file_path && is_array($activeSolicitation->file_path) && count($activeSolicitation->file_path) > 0)
                        <div class="flex flex-col {{ auth()->user()->role === 'atendente' ? 'items-start mr-auto' : 'items-end ml-auto' }} gap-2 max-w-[80%] mt-2 chat-message">
                            @foreach($activeSolicitation->file_path as $path)
                                @php
                                    $extension = pathinfo($path, PATHINFO_EXTENSION);
                                    $isImage = in_array(strtolower($extension), ['jpg', 'jpeg', 'png', 'gif', 'webp']);
                                @endphp
                                <div class="p-3 {{ auth()->user()->role === 'atendente' ? 'bg-[#EDEDED] text-gray-800 border border-gray-300/30' : 'bg-[#DA291C] text-white' }} rounded-2xl shadow-md flex flex-col gap-2 max-w-sm">
                                    @if($isImage)
                                        <a href="javascript:void(0)" onclick="openImageLightbox('{{ Storage::url($path) }}')">
                                            <img src="{{ Storage::url($path) }}" alt="Preview" class="max-w-[200px] rounded-lg hover:opacity-95 transition-opacity cursor-pointer">
                                        </a>
                                    @else
                                        <div class="p-4 flex items-center gap-3 {{ auth()->user()->role === 'atendente' ? 'bg-black/5 text-gray-800' : 'bg-white/15 text-white' }} rounded-xl">
                                            <svg class="w-8 h-8 {{ auth()->user()->role === 'atendente' ? 'text-gray-600' : 'text-white' }}" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5-3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
                                            </svg>
                                            <div class="text-left {{ auth()->user()->role === 'atendente' ? 'text-gray-800' : 'text-white' }}">
                                                <p class="text-xs font-bold truncate max-w-[150px]">{{ basename($path) }}</p>
                                                <a href="{{ Storage::url($path) }}" download class="{{ auth()->user()->role === 'atendente' ? 'text-gray-600 hover:text-gray-850' : 'text-white hover:underline' }} text-[10px] font-bold uppercase">Baixar arquivo</a>
                                            </div>
                                        </div>
                                    @endif
                                    <div class="flex justify-between items-center text-[9px] font-extrabold tracking-wider {{ auth()->user()->role === 'atendente' ? 'text-gray-500 border-gray-300/30' : 'text-white/90 border-white/10' }} uppercase border-t pt-1.5 gap-4">
                    ...
                    <div class="flex flex-col items-center justify-center text-center py-4 px-6 gap-1.5 chat-message">
                        <div class="flex items-center gap-2 text-gray-500 italic text-sm font-semibold justify-center">
                            ...
                            @if(auth()->user()->role === 'atendente')
                                <span>Chamado ID {{ $activeSolicitation->ticket_number }} atribuído a você para atendimento.</span>
                            @else
                                @if($activeSolicitation->status === 'na_fila')
                                    <span id="queue-status-inline">Chamado recebido ID {{ $activeSolicitation->ticket_number }}! Você é o {{ $queuePosition ?? 1 }}º na fila. Tempo médio de espera: 20 minutos</span>
                                @else
                                    <span>Chamado recebido ID {{ $activeSolicitation->ticket_number }}! Aguardando atualização do atendimento.</span>
                                @endif
                            @endif
                        </div>
                    ...
                    <div id="sistema-atendimento-iniciado" class="text-center py-1 {{ in_array($activeSolicitation->status, ['na_fila', 'aberta', 'nova']) ? 'hidden' : '' }} chat-message">
                        <p class="text-xs font-bold text-gray-550 italic">
                            @if(auth()->user()->role === 'atendente')
                                <span id="atendimento-iniciado-text">Você iniciou o atendimento com {{ $activeSolicitation->user->name ?? 'Cliente' }}</span>
                            @else
                                <span id="atendimento-iniciado-text">{{ $activeSolicitation->atendente->name ?? 'LUCAS R. - SUPORTE PRISMA' }} irá dar continuidade ao seu atendimento</span>
                            @endif
                        </p>
                    </div>
                                    ...
                                    <div class="videocall-card {{ $isEnded ? 'ended' : '' }}">
                                        ...
                                        @if(!$isEnded)
                                            ...
                                            @if(auth()->user()->role === 'atendente' || $isCurrentUser)
                                                <button type="button" onclick="endVideoCall({{ $msg->id }})" class="videocall-end-btn">
                                                    Encerrar chamada
                                                </button>
                                            @endif
                                        @else
                                            <span style="font-size: 11px; color: rgba(255,255,255,0.4); font-weight:700; margin-top:4px;">
                                                Encerrada por {{ $meta['ended_by'] ?? 'Sistema' }} às {{ isset($meta['ended_at']) ? date('H:i', strtotime($meta['ended_at'])) : '' }}
                                            </span>
                                        @endif
                                    </div>
                                </div>
                            @else
                                <div class="group flex items-center {{ $isCurrentUser ? 'justify-end ml-auto' : 'justify-start mr-auto' }} gap-3 max-w-[85%] relative chat-message" data-message-id="{{ $msg->id }}">

                                @if($isCurrentUser)
                                    <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                                        <button onclick="handleMenuOption('delete')" class="text-gray-400 hover:text-red-500 transition-colors p-1.5 rounded-full hover:bg-gray-100 focus:outline-none cursor-pointer" title="Apagar">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.2">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                                            </svg>
                                        </button>
                                        <button onclick="handleMenuOption('edit')" class="text-gray-400 hover:text-gray-755 transition-colors p-1.5 rounded-full hover:bg-gray-100 focus:outline-none cursor-pointer" title="Editar">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.2">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                            </svg>
                                        </button>
                                        <button onclick="replyToMessage('{{ $msg->id }}')" class="text-gray-400 hover:text-gray-750 transition-colors p-1.5 rounded-full hover:bg-gray-100 focus:outline-none cursor-pointer" title="Responder">
                                ...
                                <div class="flex flex-col {{ $isCurrentUser ? 'items-end' : 'items-start' }} gap-1 max-w-[90%] relative">
                                    <div class="relative p-4 {{ $isCurrentUser ? 'text-white bg-[#DA291C] rounded-2xl rounded-tr-none' : 'bg-[#EDEDED] text-gray-800 rounded-2xl rounded-tl-none border border-transparent' }} shadow-md flex flex-col gap-2 pr-8 message-bubble-content transition-all duration-300">
                                        <!-- Chevron button -->
                                        <button onclick="toggleMessageMenu(event, '{{ $msg->id }}')" class="absolute top-3.5 right-3 {{ $isCurrentUser ? 'text-white/70 hover:text-white' : 'text-gray-400 hover:text-gray-750' }} focus:outline-none cursor-pointer transition-opacity opacity-0 group-hover:opacity-100">
                                        ...
                                        @if($msg->parent)
                                            ...
                                            <div class="px-3 py-1.5 {{ $replyBg }} rounded-lg border-l-2 {{ $borderCol }} text-xs mb-1.5 flex flex-col opacity-85 cursor-pointer hover:opacity-100 transition-opacity" onclick="scrollToMessage('{{ $msg->parent->id }}')">
                                                <span class="font-extrabold text-[10px] {{ $replySenderClass }} uppercase leading-none mb-0.5">{{ strtoupper($msg->parent->user?->name ?? 'SISTEMA') }}</span>
                                                <span class="truncate {{ $replyTextClass }} leading-tight">{{ $msg->parent->text ?? 'Arquivo' }}</span>
                                            </div>
                                        @endif
                                        ...
                                        @if($msg->file_path)
                                            ...
                                            @endphp
                                            @if($isImage)
                                                <a href="javascript:void(0)" onclick="openImageLightbox('{{ asset('storage/' . $msg->file_path) }}')">
                                                    <img src="{{ asset('storage/' . $msg->file_path) }}" alt="Anexo" class="w-full max-w-xs object-cover rounded-xl border {{ $isCurrentUser ? 'border-white/10' : 'border-gray-300/30' }} mb-1 hover:opacity-95 transition-opacity cursor-pointer">
                                                </a>
                                            @else
                                                @php
                                                ...
                                                @endphp
                                                <a href="{{ asset('storage/' . $msg->file_path) }}" download="{{ $msg->file_name }}" class="p-3 flex items-center gap-3 {{ $iconBg }} rounded-xl mb-1 hover:opacity-95 transition-opacity">
                                        ...
                                        <div class="flex justify-between items-center text-[9px] font-extrabold tracking-wider {{ $isCurrentUser ? 'text-white/90 border-white/10' : 'text-gray-500 border-gray-300/30' }} uppercase border-t pt-1.5 gap-4">
                                    ...
                                    <div class="flex items-center gap-1 mt-1 empty:hidden message-reactions-container">
                                        @if($msg->reactions)
                                            @foreach($msg->reactions as $emoji => $users)
                                                ...
                                                @endphp
                                                <button onclick="event.stopPropagation(); removeReaction('{{ $msg->id }}', '{{ $emoji }}')" class="flex items-center gap-1.5 px-2.5 py-0.5 rounded-full {{ $userReacted ? 'bg-[#DA291C]/10 border-[#DA291C]/35 text-[#DA291C]' : 'bg-gray-100 border-gray-200 text-gray-700' }} hover:bg-gray-200 border text-xs font-bold transition-all cursor-pointer active:scale-95 focus:outline-none">
                                                    <span>{{ $emoji }}</span> <span class="text-gray-450 font-extrabold text-[10px]">{{ count($users) }}</span>
                                                </button>
                                            @endforeach
                                        @endif
                                    </div>
                    ...
                    <!-- Typing indicator -->
                    <div id="typing-indicator-wrapper" class="flex items-center gap-2 text-gray-400 select-none animate-pulse hidden">
                        @if(auth()->user()->role === 'atendente')
                            <div class="relative w-5 h-5 rounded-full bg-gray-150 flex items-center justify-center text-gray-650 border border-gray-200">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" class="w-3 h-3">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" />
                                </svg>
                            </div>
                            <span class="text-[9px] font-extrabold uppercase tracking-wider text-gray-400">{{ $activeSolicitation->user->name ?? 'Cliente' }} está digitando...</span>
                        @else
                            <img src="https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=100&auto=format&fit=crop&q=60" alt="Lucas Agent" class="w-5 h-5 rounded-full object-cover">
                            <span class="text-[9px] font-extrabold uppercase tracking-wider text-gray-400">Lucas R. está digitando...</span>
                        @endif
                    </div>
                @endif
                </div>
            ...
            <div class="px-6 py-2 relative">
                ...
                @if(auth()->user()->role === 'atendente' && $activeSolicitation->status === 'na_fila')
                    ...
                    <div id="iniciar-atendimento-wrapper" class="flex flex-col items-center justify-center py-4 bg-white select-none">
                        <button 
                            type="button"
                            onclick="iniciarAtendimentoChat({{ $activeSolicitation->id }})"
                ...
                @if(auth()->user()->role === 'user')
                    <div id="client-closure-wrapper" class="{{ $isClosedForClient ? '' : 'hidden' }} mb-3">
                        <div class="client-closure-card" style="font-family: 'AMX', sans-serif;">
                            ...
                            <p id="evaluation-sent-msg" class="text-[11px] font-bold text-green-700 {{ $existingClientEvaluation ? '' : 'hidden' }}">
                ...
                <div id="chat-input-form-wrapper" class="{{ ($activeSolicitation->status === 'na_fila' || $isClosedForClient || $isClosedForAttendant) ? 'hidden' : '' }}">
                    <!-- Reply Preview Pill -->
                <div id="reply-preview-pill" class="hidden items-center justify-between bg-white border border-gray-200 shadow-sm px-4 py-2 rounded-full text-xs font-bold text-gray-700 mb-2 w-max max-w-full animate-fade-in">
                    <div class="flex items-center gap-2 truncate">
                        ...
                        <span class="truncate text-gray-650">
                            Em resposta a <span id="reply-sender" class="font-extrabold text-[#DA291C] uppercase"></span>:
                            <span id="reply-body" class="font-semibold italic text-gray-500"></span>
                        </span>
                    </div>
                    <button type="button" onclick="cancelReply(event)" class="ml-3 text-gray-400 hover:text-gray-600 focus:outline-none cursor-pointer flex-shrink-0">
                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                    </button>
                ...
                <!-- Edit Preview Pill -->
                <div id="edit-preview-pill" class="hidden items-center justify-between bg-white border border-gray-200 shadow-sm px-4 py-2 rounded-full text-xs font-bold text-gray-700 mb-2 w-max max-w-full animate-fade-in">
                    <div class="flex items-center gap-2 truncate">
                        <svg class="w-3.5 h-3.5 text-[#DA291C] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125" />
                        </svg>
                        <span class="truncate text-gray-650">
                            Editando mensagem: <span id="edit-body" class="font-semibold italic text-gray-500"></span>
                        </span>
                    </div>
                    <button type="button" onclick="cancelEdit(event)" class="ml-3 text-gray-400 hover:text-gray-600 focus:outline-none cursor-pointer flex-shrink-0">
                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                    </button>
                ...
                <div id="attachment-preview-pill" class="hidden items-center justify-between bg-white border border-gray-200 shadow-sm px-4 py-2 rounded-full text-xs font-bold text-gray-700 mb-2 w-max max-w-full animate-fade-in">
                    <div class="flex items-center gap-2 truncate">
                        <svg class="w-3.5 h-3.5 text-[#DA291C] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.2">
                        ...
                        <span class="truncate text-gray-650">
                            ...
                            <span id="attachment-filesize" class="text-gray-400 font-semibold text-[10px] ml-1"></span>
                        </span>
                    </div>
                    <button type="button" onclick="cancelAttachment(event)" class="ml-3 text-gray-400 hover:text-gray-600 focus:outline-none cursor-pointer flex-shrink-0">
                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                    </button>
                ...
                <form id="chat-input-form" onsubmit="sendChatMessage(event)" class="m-0 flex items-center justify-between gap-3 px-6 py-3 rounded-full bg-[#EDEDED] focus-within:ring-1 focus-within:ring-[#DA291C]/30 transition-all">
                    ...
                    <div class="flex items-center gap-3 text-gray-500 flex-shrink-0">
                        ...
                        <button type="button" onclick="toggleInputEmojiPicker(event)" class="hover:text-gray-800 transition-colors focus:outline-none cursor-pointer">
                        ...
                        @if(auth()->user()->role === 'atendente')
                            ...
                            <button type="button" onclick="toggleQuickRepliesMenu(event)" class="hover:text-gray-800 transition-colors focus:outline-none cursor-pointer" title="Respostas rápidas">
                                <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                        ...
                        @endif
                        <!-- Attachment trigger -->
                        <button type="button" onclick="triggerFileInput()" class="hover:text-gray-800 transition-colors focus:outline-none cursor-pointer">
                            <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="m18.375 12.739-7.693 7.693a4.5 4.5 0 0 1-6.364-6.364l10.94-10.94A3 3 0 1 1 19.5 7.372L8.552 18.32m.009-.01-.01.01m5.699-9.941-7.81 7.81a1.5 1.5 0 0 0 2.112 2.13" />
                            </svg>
                        </button>
            ...
            <div id="input-emoji-picker" class="fixed hidden z-[95] bg-[#303030] rounded-2xl shadow-2xl p-3 flex flex-wrap gap-2 border border-white/10 w-48 justify-center select-none animate-fade-in">
                <button type="button" onclick="insertEmoji('😀')" class="text-lg hover:scale-125 transition-transform focus:outline-none cursor-pointer">😀</button>
                <button type="button" onclick="insertEmoji('😂')" class="text-lg hover:scale-125 transition-transform focus:outline-none cursor-pointer">😂</button>
                <button type="button" onclick="insertEmoji('❤️')" class="text-lg hover:scale-125 transition-transform focus:outline-none cursor-pointer">❤️</button>
                <button type="button" onclick="insertEmoji('👍')" class="text-lg hover:scale-125 transition-transform focus:outline-none cursor-pointer">👍</button>
                <button type="button" onclick="insertEmoji('🔥')" class="text-lg hover:scale-125 transition-transform focus:outline-none cursor-pointer">🔥</button>
                <button type="button" onclick="insertEmoji('🎉')" class="text-lg hover:scale-125 transition-transform focus:outline-none cursor-pointer">🎉</button>
                <button type="button" onclick="insertEmoji('🙏')" class="text-lg hover:scale-125 transition-transform focus:outline-none cursor-pointer">🙏</button>
            ...
            @if(auth()->user()->role === 'atendente')
                <div id="quick-replies-menu" class="fixed hidden z-[98] w-[300px] bg-[#303030] text-white rounded-2xl shadow-2xl p-3 border border-white/10 select-none animate-fade-in">
                    <div class="relative mb-2">
                        <input id="quick-replies-search" type="text" placeholder="Pesquisar" class="w-full h-8 rounded-full bg-[#3A3A3F] border border-white/20 pl-3 pr-9 text-xs placeholder-white/45 focus:outline-none">
                        <svg class="w-3.5 h-3.5 text-white/45 absolute right-3 top-2.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="m21 21-4.35-4.35m1.85-5.15a7 7 0 1 1-14 0 7 7 0 0 1 14 0Z" />
                        </svg>
                    </div>
                    <div id="quick-replies-menu-list" class="max-h-56 overflow-y-auto space-y-1 pr-1 chat-scroll-clean"></div>
                    <button type="button" onclick="openQuickRepliesEditor()" class="mt-2 w-full text-center text-[10px] font-bold text-white/70 hover:text-white transition-colors">
            ...
            <!-- Tag Menu (Atendente) -->
            <div id="ticket-tag-menu" class="fixed hidden z-[97] w-60 bg-[#3A3A3F] text-white rounded-2xl shadow-2xl p-3 border border-white/10 select-none animate-fade-in">
                <button type="button" onclick="openTagEditorModal()" class="w-full flex items-center gap-2 px-2 py-1.5 text-left text-[17px] font-bold hover:bg-white/10 rounded-lg transition-colors">
            ...
            <!-- Add/Edit Tag Modal -->
            <div id="tag-editor-modal" class="fixed inset-0 z-[110] hidden bg-black/60 backdrop-blur-sm items-center justify-center p-4 select-none">
                <div id="tag-editor-modal-content" class="bg-white w-full max-w-[620px] rounded-[24px] shadow-2xl p-7 relative flex flex-col gap-5 transform scale-95 opacity-0 transition-all duration-300">
                    <button type="button" onclick="closeTagEditorModal()" class="absolute top-5 right-5 text-gray-500 hover:text-gray-700 transition-colors">
                    ...
                    <h3 class="text-[30px] font-extrabold tracking-tight text-[#DA291C]" style="font-family: 'AMX', sans-serif;">Adicionar tag</h3>

                    <input id="tag-name-input" type="text" placeholder="Nome da tag" maxlength="40" class="w-full h-14 px-4 border border-gray-200 rounded-xl text-sm placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-[#DA291C]">

                    <div class="flex flex-wrap items-center gap-2" id="tag-color-palette"></div>

                    <div class="text-sm font-bold text-[#A01724]" style="font-family: 'AMX', sans-serif;">Personalizar cor</div>

                    <button type="button" onclick="saveTagFromModal()" class="w-full h-14 rounded-2xl text-white text-2xl font-extrabold tracking-tight" style="font-family: 'AMX', sans-serif; background: linear-gradient(89.24deg, #A01724 0%, #DA291C 100%);">
                        Adicionar tag
                    </button>

                    <button type="button" onclick="closeTagEditorModal()" class="text-gray-600 hover:text-gray-800 text-xl font-medium transition-colors" style="font-family: 'AMX', sans-serif;">
                        Cancelar
                    </button>
                </div>
            </div>
            ...
            <div id="quick-reply-add-modal" class="fixed inset-0 z-[112] hidden bg-black/60 backdrop-blur-sm items-center justify-center p-4 select-none">
                <div id="quick-reply-add-modal-content" class="bg-white w-full max-w-[640px] rounded-[18px] shadow-2xl p-6 relative flex flex-col gap-3 transform scale-95 opacity-0 transition-all duration-300">
                    <button type="button" onclick="closeQuickReplyAddModal()" class="absolute top-4 right-4 text-black hover:text-gray-700 transition-colors">
                    ...
                    <input id="quick-reply-add-name" type="text" placeholder="Nome da resposta/atalho (disponível só para você)" maxlength="80" class="w-full h-12 rounded-lg border border-gray-300 px-3 text-sm text-gray-700 placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-[#DA291C]">

                    <textarea id="quick-reply-add-text" placeholder="Resposta rápida" class="w-full min-h-[110px] rounded-lg border border-gray-300 px-3 py-3 text-sm text-gray-700 placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-[#DA291C]"></textarea>

                    <button type="button" onclick="submitQuickReplyAdd()" class="w-full h-12 rounded-xl text-white text-xl font-extrabold hover:opacity-95 transition-all" style="background: linear-gradient(89.24deg, #A01724 0%, #DA291C 100%);">
                        Adicionar resposta rápida
                    </button>

                    <button type="button" onclick="closeQuickReplyAddModal()" class="text-gray-700 text-base font-medium hover:text-gray-900 transition-colors">
                        Cancelar
                    </button>
                </div>
            </div>
            ...
            <div id="quick-reply-edit-modal" class="fixed inset-0 z-[112] hidden bg-black/60 backdrop-blur-sm items-center justify-center p-4 select-none">
                <div id="quick-reply-edit-modal-content" class="bg-white w-full max-w-[640px] rounded-[18px] shadow-2xl p-6 relative flex flex-col gap-3 transform scale-95 opacity-0 transition-all duration-300">
                    <button type="button" onclick="closeQuickReplyEditModal()" class="absolute top-4 right-4 text-black hover:text-gray-700 transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.2" stroke="currentColor" class="w-4 h-4">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" />
                        </svg>
                    </button>
                    ...
                    <input id="quick-reply-edit-name" type="text" placeholder="Nome da resposta/atalho" maxlength="80" class="w-full h-12 rounded-lg border border-gray-300 px-3 text-sm text-gray-700 placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-[#DA291C]">

                    <textarea id="quick-reply-edit-text" placeholder="Resposta rápida" class="w-full min-h-[110px] rounded-lg border border-gray-300 px-3 py-3 text-sm text-gray-700 placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-[#DA291C]"></textarea>

                    <button type="button" onclick="submitQuickReplyEdit()" class="w-full h-12 rounded-xl text-white text-xl font-extrabold hover:opacity-95 transition-all" style="background: linear-gradient(89.24deg, #A01724 0%, #DA291C 100%);">
                        Salvar alterações
                    </button>

                    <button type="button" onclick="closeQuickReplyEditModal()" class="text-gray-700 text-base font-medium hover:text-gray-900 transition-colors">
                        Cancelar
                    </button>
                </div>
            </div>
            ...
            @if(auth()->user()->role === 'atendente')
                <div id="checklist-modal" class="fixed inset-0 z-[113] hidden bg-black/60 backdrop-blur-sm items-center justify-center p-4 select-none">
                    <div id="checklist-modal-content" class="checklist-figma-modal w-full max-w-[596px] rounded-[24px] shadow-2xl p-6 md:p-7 relative flex flex-col gap-6 transform scale-95 opacity-0 transition-all duration-300">
                        <button type="button" onclick="closeChecklistModal()" class="absolute top-5 right-5 text-gray-500 hover:text-gray-700 transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.2" stroke="currentColor" class="w-5 h-5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" />
                            </svg>
                        </button>
                        ...
                        <button type="button" onclick="closeChecklistModal()" class="checklist-cancel-btn hover:text-gray-700 transition-colors" style="font-family: 'AMX', sans-serif;">
                            Cancelar
                        </button>
                    </div>
                </div>
            ...
            @if(auth()->user()->role === 'user')
                <div id="evaluation-modal" class="fixed inset-0 z-[114] hidden bg-black/60 backdrop-blur-sm items-center justify-center p-4 select-none">
                    <div id="evaluation-modal-content" class="evaluation-modal-shell shadow-2xl transform scale-95 opacity-0 transition-all duration-300" style="font-family: 'AMX', sans-serif;">
                        <button type="button" onclick="closeEvaluationModal()" class="absolute top-4 right-4 text-gray-500 hover:text-gray-700 transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.2" stroke="currentColor" class="w-5 h-5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" />
                            </svg>
                        </button>
                        ...
                        <div class="evaluation-stars-wrap" id="evaluation-stars-wrap">
                            <button type="button" class="evaluation-star" data-rating="1" onclick="setEvaluationRating(1)">
                                <svg viewBox="0 0 24 24" fill="currentColor" class="w-full h-full"><path d="M12 2.5l2.95 5.98 6.6.96-4.78 4.66 1.13 6.57L12 17.57l-5.9 3.1 1.13-6.57L2.45 9.44l6.6-.96L12 2.5z"/></svg>
                            </button>
                        ...
                        <div class="evaluation-question-row">
                            ...
                            <button type="button" id="evaluation-resolved-no" onclick="setEvaluationResolved(false)" class="evaluation-choice-btn">
                                <span class="evaluation-choice-dot" id="evaluation-resolved-no-dot"></span>
                                <span>Não foi resolvido</span>
                            </button>
                            <button type="button" id="evaluation-resolved-yes" onclick="setEvaluationResolved(true)" class="evaluation-choice-btn">
                                <span class="evaluation-choice-dot" id="evaluation-resolved-yes-dot"></span>
                                <span>Sim, foi resolvido</span>
                            </button>
                        </div>

                        <textarea id="evaluation-comment" class="w-full min-h-[74px] rounded-[8px] border border-[#B9BEC8] bg-[#F6F6F6] px-3 py-2.5 text-[12px] text-gray-700 placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-[#DA291C]" placeholder="Deixe um comentário sobre o atendimento recebido"></textarea>

                        <button id="evaluation-submit-btn" type="button" onclick="submitEvaluation()" disabled class="evaluation-submit-btn">
                            Enviar avaliação
                        </button>

                        <button type="button" onclick="closeEvaluationModal()" class="evaluation-cancel-btn hover:text-gray-900 transition-colors">
                            Cancelar
                        </button>
                    </div>
                </div>
            ...
            <div id="message-context-menu" class="fixed hidden z-[90] w-52 bg-[#303030] text-white rounded-2xl shadow-2xl py-2 flex flex-col gap-0.5 select-none transition-all duration-100" style="font-family: 'AMX', sans-serif;">
                <button onclick="handleMenuOption('info')" class="flex items-center gap-3 px-4 py-2 text-xs font-bold text-left hover:bg-white/10 transition-colors w-full cursor-pointer">
                    <svg class="w-4 h-4 text-white/80" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.2">
                ...
                <button onclick="handleMenuOption('edit')" class="flex items-center gap-3 px-4 py-2 text-xs font-bold text-left hover:bg-white/10 transition-colors w-full cursor-pointer">
                    <svg class="w-4 h-4 text-white/80" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125" />
                    </svg>
                    <span>Editar</span>
                </button>
                <button onclick="handleMenuOption('reply')" class="flex items-center gap-3 px-4 py-2 text-xs font-bold text-left hover:bg-white/10 transition-colors w-full cursor-pointer">
                    <svg class="w-4 h-4 text-white/80" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 15 3 9m0 0 6-6M3 9h12a6 6 0 0 1 0 12h-3" />
                    </svg>
                    <span>Responder</span>
                </button>
                <button onclick="handleMenuOption('react_trigger', event)" id="menu-react-btn" class="flex items-center gap-3 px-4 py-2 text-xs font-bold text-left hover:bg-white/10 transition-colors w-full cursor-pointer justify-between">
                    <div class="flex items-center gap-3">
                        <svg class="w-4 h-4 text-white/80" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15.182 15.182a4.5 4.5 0 0 1-6.364 0M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0ZM9.75 9.75c0 .414-.168.75-.375.75S9 10.164 9 9.75 9.168 9 9.375 9s.375.336.375.75Zm-.375 0h.008v.015h-.008V9.75Zm5.625 0c0 .414-.168.75-.375.75s-.375-.336-.375-.75.168-.75.375-.75.375.336.375.75Zm-.375 0h.008v.015h-.008V9.75Z" />
                        </svg>
                ...
                <button onclick="handleMenuOption('copy')" class="flex items-center gap-3 px-4 py-2 text-xs font-bold text-left hover:bg-white/10 transition-colors w-full cursor-pointer">
                    <svg class="w-4 h-4 text-white/80" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 17.25v3.375c0 .621-.504 1.125-1.125 1.125h-9.75a1.125 1.125 0 01-1.125-1.125V7.875c0-.621.504-1.125 1.125-1.125H5.25m11.9-3.664A2.251 2.251 0 0015 2.25h-1.5a2.251 2.251 0 00-2.15 1.586m5.8 0c.065.21.1.433.1.664v.75h-6V4.5c0-.231.035-.454.1-.664M19.5 3.577a2.25 2.25 0 011.955 2.186v11.487m0 0a2.25 2.25 0 01-1.955 2.186m0-15.859A2.25 2.25 0 0018 2.25h-1.5A2.25 2.25 0 0014.25 4.5v.75m4.5 0a2.25 2.25 0 01-2.25 2.25h-3a2.25 2.25 0 01-2.25-2.25v-.75" />
                    </svg>
                    <span>Copiar</span>
                </button>
                <button onclick="handleMenuOption('delete')" class="flex items-center gap-3 px-4 py-2 text-xs font-bold text-left hover:bg-white/10 text-red-400 hover:text-red-350 transition-colors w-full cursor-pointer">
                    <svg class="w-4 h-4 text-red-400/80" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                    </svg>
                    <span>Apagar</span>
                </button>
            ...
            <div id="emoji-reaction-popup" class="fixed hidden z-[95] bg-[#303030] rounded-full shadow-2xl px-3 py-1.5 flex items-center gap-2 border border-white/10">
                <button onclick="handleReactionSelect('👍')" class="text-base hover:scale-125 transition-transform focus:outline-none cursor-pointer">👍</button>
                <button onclick="handleReactionSelect('❤️')" class="text-base hover:scale-125 transition-transform focus:outline-none cursor-pointer">❤️</button>
                <button onclick="handleReactionSelect('😂')" class="text-base hover:scale-125 transition-transform focus:outline-none cursor-pointer">😂</button>
                <button onclick="handleReactionSelect('😮')" class="text-base hover:scale-125 transition-transform focus:outline-none cursor-pointer">😮</button>
                <button onclick="handleReactionSelect('😢')" class="text-base hover:scale-125 transition-transform focus:outline-none cursor-pointer">😢</button>
                <button onclick="handleReactionSelect('🙏')" class="text-base hover:scale-125 transition-transform focus:outline-none cursor-pointer">🙏</button>
            ...
            <div id="msg-info-modal" class="fixed inset-0 z-[100] hidden bg-black/60 backdrop-blur-sm items-center justify-center p-4 select-none">
                <div id="msg-info-modal-content" class="bg-white w-full max-w-[400px] rounded-2xl shadow-2xl p-6 relative flex flex-col gap-4 transform scale-95 opacity-0 transition-all duration-300">
                    <button onclick="closeMsgInfoModal()" class="absolute top-4 right-4 text-gray-400 hover:text-gray-600 transition-colors focus:outline-none cursor-pointer">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-4 h-4">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" />
                        </svg>
                    </button>
                    <h3 class="text-base font-extrabold text-[#DA291C] uppercase tracking-wider" style="font-family: 'AMX', sans-serif;">Dados da mensagem</h3>
                    <div class="flex flex-col gap-3 text-sm text-gray-700 font-semibold" style="font-family: 'AMX', sans-serif;">
                        ...
                        <div class="flex flex-col">
                            <span class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Status da entrega</span>
            ...
            <div id="toast-container" class="fixed top-5 left-1/2 -translate-x-1/2 z-[150] pointer-events-none flex flex-col gap-2"></div>
        @else
            <!-- Estado Vazio -->
            <div class="flex-1 flex flex-col items-center justify-center p-6 text-center select-none">
                ...
                <p class="text-sm text-gray-500 max-w-sm">Abra o menu lateral ou clique no botão de suporte para iniciar uma nova conversa.</p>
            </div>
        @endif
    ...
    @if($activeSolicitation)
        <div class="w-[320px] lg:w-[350px] bg-white rounded-[24px] shadow-md flex flex-col border border-gray-200/80 p-6 pt-10 gap-6 flex-shrink-0 select-none overflow-y-auto hidden lg:flex chat-scroll-clean min-h-0">
            <div id="right-sidebar-default-content" class="contents">
                ...
                <div class="text-center pb-6 border-b border-gray-250">
                    ...
                    @if(!$rightAvatarUser && auth()->user()->role === 'user')
                        <!-- Ícone de ticket padrão Claro se o cliente ainda não tiver atendente -->
                        <div class="w-20 h-20 rounded-full bg-white flex items-center justify-center flex-shrink-0 border p-4 shadow-sm mx-auto mb-4">
                            <img src="/icones/Icone Ticket.png" alt="Ticket" class="w-full h-full object-contain">
                        </div>
                    @else
                        <div class="relative w-20 h-20 mx-auto mb-4 flex-shrink-0">
                            <img src="https://ui-avatars.com/api/?name={{ urlencode($rightAvatarName) }}&background={{ $rightAvatarBg }}&color={{ $rightAvatarColor }}&bold=true&rounded=true" alt="Avatar" class="w-20 h-20 rounded-full object-cover border border-gray-255 shadow-sm">
                        </div>
                    @endif
                ...
                <div id="sidebar-attachments-container" class="flex items-center gap-2 overflow-x-auto pb-2">
                    ...
                    @foreach($solicitationFiles ?? [] as $path)
                        ...
                        @endphp
                        @if($isImage)
                            <a href="javascript:void(0)" onclick="openImageLightbox('{{ Storage::url($path) }}')" class="w-[84px] aspect-square rounded-xl overflow-hidden border border-gray-250 hover:opacity-90 transition-all flex-shrink-0">
                                <img src="{{ Storage::url($path) }}" alt="Preview" class="w-full h-full object-cover">
                            </a>
                        @else
                            <a href="{{ Storage::url($path) }}" download class="w-[84px] aspect-square rounded-xl bg-gray-50 flex flex-col items-center justify-center border border-gray-250 hover:bg-gray-100 transition-colors flex-shrink-0">
                    ...
                    @foreach($messageFiles ?? [] as $path)
                        ...
                        @endphp
                        @if($isImage)
                            <a href="javascript:void(0)" onclick="openImageLightbox('{{ asset('storage/' . $path) }}')" class="w-[84px] aspect-square rounded-xl overflow-hidden border border-gray-250 hover:opacity-90 transition-all flex-shrink-0">
                                <img src="{{ asset('storage/' . $path) }}" alt="Preview" class="w-full h-full object-cover">
                            </a>
                        @else
                            <a href="{{ asset('storage/' . $path) }}" download class="w-[84px] aspect-square rounded-xl bg-gray-50 flex flex-col items-center justify-center border border-gray-250 hover:bg-gray-100 transition-colors flex-shrink-0">
                                <svg class="w-6 h-6 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5-3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
                                </svg>
                                <span class="text-[8px] font-bold text-gray-500 uppercase mt-1">DOC</span>
            ...
            @if(auth()->user()->role === 'atendente')
                <div id="quick-replies-editor" class="hidden flex-col gap-3 pt-1 relative flex-1 min-h-0">
                    ...
                    <div class="relative mt-1">
                        <input id="quick-replies-editor-search" type="text" placeholder="Pesquisar" class="w-full h-10 rounded-full border border-gray-400 bg-transparent px-4 pr-10 text-sm placeholder-gray-600 focus:outline-none focus:ring-1 focus:ring-[#DA291C]">
                        <svg class="w-4 h-4 text-gray-600 absolute right-4 top-3.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="m21 21-4.35-4.35m1.85-5.15a7 7 0 1 1-14 0 7 7 0 0 1 14 0Z" />
                        </svg>
                    </div>
                    ...
                    <button type="button" onclick="openQuickReplyAddModal()" class="w-full h-10 rounded-xl text-white text-base font-extrabold hover:opacity-95 transition-all" style="background: linear-gradient(89.24deg, #A01724 0%, #DA291C 100%);">
                        Adicionar resposta rápida
                    </button>
                </div>
            @endif
...
<div id="image-lightbox-modal" class="hidden fixed inset-0 z-[100] bg-black/90 backdrop-blur flex flex-col items-center justify-center p-4 transition-all duration-300">
    <button type="button" onclick="closeImageLightbox()" class="absolute top-6 right-6 text-white hover:text-gray-300 transition-colors p-2 focus:outline-none cursor-pointer">
    ...
    <div class="relative max-w-[90vw] max-h-[85vh] flex items-center justify-center">
        <img id="lightbox-image-el" src="" alt="Visualização" class="max-w-full max-h-full object-contain rounded-xl shadow-2xl border border-white/10">
    </div>
    <div class="mt-4 text-center">
        <a id="lightbox-download-link" href="" download class="px-5 py-2 bg-white/10 hover:bg-white/20 border border-white/20 text-white rounded-full text-xs font-extrabold uppercase tracking-wider transition-all inline-flex items-center gap-2">
...
<script>
    ...
    function endVideoCall(messageId) {
        if (!confirm('Deseja realmente encerrar esta chamada de vídeo?')) return;

        fetch(`/videocall/${messageId}/end`, {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json',
        ...
        .then(data => {
            if (data.success) {
                ...
                alert(data.error || 'Erro ao encerrar chamada.');
            }
        })
        .catch(err => {
            console.error('Erro ao encerrar chamada:', err);
            alert('Erro de conexão ao encerrar.');
        });
    }
